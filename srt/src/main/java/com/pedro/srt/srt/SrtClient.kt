...
class SrtClient(private val connectChecker: ConnectChecker) {
  ...
  private var configuredPassphrase = ""
  private var configuredEncryptionType = EncryptionType.NONE
  ...
  fun setPassphrase(passphrase: String, type: EncryptionType) {
    if (!isStreaming) {
      validateSrtPassphrase(passphrase)
      configuredPassphrase = passphrase
      configuredEncryptionType = if (passphrase.isEmpty()) EncryptionType.NONE else type
    }
  }
  ...
  fun connect(url: String?, isRetry: Boolean = false) {
    ...
    if (!isStreaming || isRetry) {
      ...
      job = scope.launch {
        ...
        val options = try {
          parseSrtConnectionOptions(urlParser)
        } catch (error: IllegalArgumentException) {
          // Fail closed. Never retain a previous connection's key or silently
          // downgrade an invalid passphrase to plaintext.
          commandsManager.setPassphrase("", EncryptionType.NONE)
          isStreaming = false
          onMainThread {
            connectChecker.onConnectionFailed(error.message ?: "Invalid SRT options")
          }
          return@launch
        }
        val host = urlParser.host
        val port = urlParser.port ?: 8888
        val path = options.streamId
        commandsManager.latency = options.latency ?: commandsManager.latency
        applyConnectionEncryption(
          commandsManager,
          options,
      ...
      is ControlPacket -> {
        when (srtPacket) {
          ...
          is Nak -> {
            ...
            this.packetsLost = (this.packetsLost.toLong() + srtPacket.getLostCount())
