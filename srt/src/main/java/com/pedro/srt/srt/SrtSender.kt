...
class SrtSender(
  ...
  private val limitSize: Int
    get() {
      // ROAM PATCH: keep SRT datagrams below the IPv6 minimum path MTU.
      // The peer can replace MTU during the handshake, so this cap belongs
      // here rather than in Constants.
      return minOf(
        commandsManager.MTU - SrtPacket.headerSize,
        6 * MpegTsPacketizer.packetSize,
      )
    }
