...
class NakTest {
  ...
  @Test
  fun `GIVEN a maximum nak range WHEN count is read THEN saturate without allocating the range`() {
    val packet = Nak()
    packet.addLostPacketsRange(0, Int.MAX_VALUE)

    assertEquals(listOf(0 to Int.MAX_VALUE), packet.getNakRanges())
    assertEquals(Int.MAX_VALUE, packet.getLostCount())
  }
}
