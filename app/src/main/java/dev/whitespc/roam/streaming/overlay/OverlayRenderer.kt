 ...
 *
 * Position handling: items store a 0 to 100 x/y position. User overlays (text,
 * image) are placed precisely via [applyPrecisePosition] (0 = flush start edge,
 * 50 = centred, 100 = flush end edge). The watermark stays on the legacy 3 x 3
 * anchor snap ([applyPosition]) so the branding can't drift. Web overlays ignore
 * position because they render full-frame.
 */
class OverlayRenderer(
    ...
    fun applyScene(scene: Scene) {
        ...
        // Decide whether the active scene needs compass or GPS data. TokenSource
    ...
    private fun createImageFilter(item: OverlayItem, source: OverlaySource.Image): ImageObjectFilterRender? =
        runCatching {
            val bitmap = decodeDownsampled(source.path) ?: return@runCatching null
            ImageObjectFilterRender().apply {
                setImage(bitmap)
                setScale(item.widthPercent, item.heightPercent)
    ...
    /** Downsample images near the stream frame size to stay inside common GL
     *  texture and heap limits. */
    ...
    /** Maps a center-coordinate percentage to RootEncoder's nearest anchor. */
     ...
     * 0 to 100 position: 0 = flush to the start edge, 50 = centred, 100 = flush to the
