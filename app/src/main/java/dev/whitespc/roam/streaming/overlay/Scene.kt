...
sealed interface OverlaySource {
    ...
    /** A web page composited into the broadcast. [url] is an HTTPS address or
     *  an app-managed reference returned by [OverlayWebStore]. Rendered
     *  full-frame: the page itself decides where its content sits. Heavier than
     *  text/image because it runs a live WebView. */
    data class WebPage(
        val url: String = "",
    ) : OverlaySource
