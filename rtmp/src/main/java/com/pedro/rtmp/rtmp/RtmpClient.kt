...
class RtmpClient(private val connectChecker: ConnectChecker) {
  ...
  // Fail closed for RTMPS certificate identity checks unless a caller makes
  // the explicit, unsafe choice to disable verification.
  var tlsHostVerification = true
            ...
            try {
              ...
              when (commandName) {
                ...
                "releaseStream", "FCPublish" -> {
                  // Server descriptions can echo a stream key or auth data.
                  Log.e(TAG, "$commandName failed")
                }
                else -> {
                  onMainThread {
