...
class TcpTunneledSocket(private val host: String, private val port: Int, private val secured: Boolean): RtmpSocket() {
  ...
  private fun configureSocket(path: String, secured: Boolean): HttpURLConnection {
    ...
    // The path contains the RTMPT session identifier. Keep it out of logcat.
    Log.i(TAG, "open tunneled request")
    ...
    synchronized(sync) {
      Thread {
        try {
          ...
          Log.e(TAG, "Close request failed (${e.javaClass.simpleName})")
        } finally {
          index.set(0)
          connectionId = ""
        }
